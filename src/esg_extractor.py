...
class ESGExtractor:
    ...
    def __init__(self, vector_db_path: str = None, enable_llm: bool = True, auto_dedupe: bool = True):
        ...
        self.keyword_config = KeywordConfig()  # 使用修正的KeywordConfig類
    ...
    def _init_llm(self):
        ...
        try:
            ...
            try:
                ...
                # 如果api_manager不可用，使用單API模式
                print("🔑 使用單API模式（api_manager不可用）")
                self.llm = ChatGoogleGenerativeAI(
                    model=GEMINI_MODEL,
    ...
    def _call_llm(self, prompt: str) -> str:
        """統一的LLM調用介面"""
        if hasattr(self, 'llm_mode') and self.llm_mode == "multi_api":
            # 使用多API管理器
    ...
    def enhanced_stage2_filtering(self, documents: List[Document]) -> List[NumericExtraction]:
        """增強的第二階段篩選：提取包含數值或百分比的內容"""
        print("🔢 執行增強第二階段篩選...")
    ...
    def run_complete_extraction(self, max_documents: int = 200) -> Tuple[List[NumericExtraction], ProcessingSummary, str]:
        ...
        # 3. 第二階段篩選
        stage2_extractions = self.enhanced_stage2_filtering(stage1_docs)
